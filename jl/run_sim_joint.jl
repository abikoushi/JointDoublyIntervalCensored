...
function simfunc(iter, sigma, tau, WE, WS, tdist)
    ...
    for j in 1:iter
        ...
        pv_incubation[:,j] = jointest.simulated_pvalue(Rs, res.value, qs, tvalue)
...
out_pv_incubation = zeros(length(tdists), length(sigmas), length(taus), length(WEs), 4, iter)
out_pv_intensity  = zeros(length(tdists), length(sigmas), length(taus), length(WEs), 4, iter) 
out_pv_B = zeros(length(tdists), length(sigmas), length(taus), length(WEs), iter)
out_vb = []
out_em = []
...
for i in eachindex(sigmas)
    for j in eachindex(taus)
        for k in eachindex(WEs)
            prop, prop_em, pv_incubation, pv_intensity, pv_B = simfunc(iter, sigmas[i], taus[j], WEs[k], WS, tdists[l])
            append!(out_vb, prop)
