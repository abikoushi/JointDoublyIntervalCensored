...
pl <- vector("list", 3)
for(i in 1:3){
  pl[[i]] <- ggplot(df_int_2_s[[i]], aes(x=pv))+
    stat_ecdf()+
    geom_abline(slope = 1, intercept = 0, colour="grey", linetype=2)+
    facet_grid(tau+sigma ~ quantile, labeller = label_both)+
    scale_x_continuous(n.breaks = 11)+
    ...
    theme(strip.text.y = element_text(angle=0), axis.text.x = element_text(angle=90))  
  ggsave(plot = pl[[i]], filename = paste0("pv_infection_mixture_",i,".pdf"), width=10, height = 10)
}

p <- pl[[1]]+pl[[2]]+pl[[3]]
print(p)
ggsave(plot = p, filename = paste0("pv_infection_mixture.tiff"), width=30, height = 10)
...
df_inc_1_s = split(df_inc1, df_inc1$WS)
df_inc_2_s = split(df_inc2, df_inc2$WS)
#pdf("pvalue_ccdf.pdf", width=10, height=10)
pl <- vector("list", 3)
for(i in 1:3){
  pl[[i]] <- ggplot(df_inc_1_s[[i]], aes(x=pv))+
    stat_ecdf()+
    geom_abline(slope = 1, intercept = 0, colour="grey", linetype=2)+
    facet_grid(tau+sigma ~ quantile, labeller = label_both)+
    scale_x_continuous(n.breaks = 11)+
    ...
    theme(strip.text.y = element_text(angle=0), axis.text.x = element_text(angle=90))  
  print(pl[[i]])  
  ggsave(plot = pl[[i]], filename = paste0("pv_incubation_lnorm_",i,".pdf"), width=10, height = 10)
}

p <- pl[[1]]+pl[[2]]+pl[[3]]
print(p)
ggsave(plot = p, filename = "pv_incubation_lnorm.tiff", width=30, height = 10)


pl <- vector("list", 3)
for(i in 1:3){
  pl[[i]] <- ggplot(df_inc_2_s[[i]], aes(x=pv))+
    stat_ecdf()+
    geom_abline(slope = 1, intercept = 0, colour="grey", linetype=2)+
    facet_grid(tau+sigma ~ quantile, labeller = label_both)+
    scale_x_continuous(n.breaks = 11)+
    ...
    theme(strip.text.y = element_text(angle=0), axis.text.x = element_text(angle=90))
  print(pl[i])  
  ggsave(plot = pl[[i]], filename = paste0("pv_incubation_mixture_",i,".pdf"), width=10, height = 10)
}

p <- pl[[1]]+pl[[2]]+pl[[3]]
print(p)
ggsave(plot = p, filename = "pv_incubation_mixture.tiff", width=30, height = 10)
