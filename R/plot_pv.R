...
for(i in 1:3){
p_i <- ggplot(df_int_1_s[[i]], aes(x=2*pv))+
  stat_ecdf()+
  geom_abline(slope = 1, intercept = 0, colour="grey", linetype=2)+
...
for(i in 1:3){
  p_i <- ggplot(df_int_2_s[[i]], aes(x=2*pv))+
    stat_ecdf()+
    geom_abline(slope = 1, intercept = 0, colour="grey", linetype=2)+
    facet_grid(tau+sigma ~ quantile, labeller = label_both)+
    scale_x_continuous(n.breaks = 11)+
...
for(i in 1:3){
  p_i <- ggplot(df_inc_1_s[[i]], aes(x=2*pv))+
    stat_ecdf()+
    geom_abline(slope = 1, intercept = 0, colour="grey", linetype=2)+
    facet_grid(tau+sigma ~ quantile, labeller = label_both)+
    scale_x_continuous(n.breaks = 11)+
...
for(i in 1:3){
  p_i <- ggplot(df_inc_2_s[[i]], aes(x=2*pv))+
    stat_ecdf()+
    geom_abline(slope = 1, intercept = 0, colour="grey", linetype=2)+
    facet_grid(tau+sigma ~ quantile, labeller = label_both)+
    scale_x_continuous(n.breaks = 11)+
